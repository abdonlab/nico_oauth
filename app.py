...
def ensure_session_defaults():
    ...
    # 💥 CORRECCIÓN: Eliminamos 'trigger_run'
...
def gemini_generate(prompt: str, temperature: float, top_p: float, max_tokens: int) -> str:
    ...
    try:
        ...
        # Esto incluye errores 4xx o 5xx del API
        st.error(f"Error en la llamada a Gemini (API): {e}")
...
# ============================================================
# LÓGICA DE PROCESAMIENTO (Función central de Chat)
# ============================================================

def process_chat_message():
    """
    Función que maneja la lógica de envío y procesamiento de mensajes, 
    garantizando que se ejecute una sola vez por interacción.
    """
    # 1. Obtener mensaje del usuario ANTES de que el input se limpie
    user_msg = st.session_state["input_val"]
    
    if not user_msg.strip():
        # Esto no debería pasar si se llama correctamente, pero es una protección
        return

    # 2. **CORRECCIÓN CLAVE:** Limpiamos el input_val inmediatamente 
    # para que en el siguiente rerun ya esté vacío, evitando re-envíos.
    st.session_state["input_val"] = ""

    # 3. Guardar mensaje de usuario
    st.session_state["history"].append({"role": "user", "content": user_msg})

    # 4. Video Aleatorio (Actualizamos el video en la barra lateral)
    try:
        ...
        if video_files:
            ...
            # Nota: El contenedor de video se actualizará en la próxima ejecución del script
    except Exception as e:
    ...
    # 6. Llamar a Gemini (Esta es la parte lenta)
    ...
    # 9. Forzar rerun para mostrar la respuesta y el input vacío
    st.rerun()
...
with video_col:
    ...
    # Lógica para mostrar el video actual
    if st.session_state["current_video"]:
        video_container.markdown(st.session_state["current_video"], unsafe_allow_html=True)
    # Si aún no hay video, inicializamos uno (Esto solo ocurre en el primer ciclo)
...
with conv_col:
    ...
    # La función process_chat_message() ahora contiene toda la lógica de generación
    # y llama a st.rerun al final para actualizar la interfaz.
    
    def action_clear():
        """Limpia el texto y fuerza un rerun para limpiar la caja visualmente"""
        st.session_state["input_val"] = ""
        st.rerun() 

    # Input con on_change. Al presionar Enter, se llama a process_chat_message()
    st.text_input(
        "Escribe tu pregunta:", 
        key="input_val", 
        on_change=process_chat_message # Llama a la función principal de chat
    )

    # Botones lado a lado
    btn_c1, btn_c2, _ = st.columns([0.15, 0.15, 0.7])
    with btn_c1:
        # El botón también llama a process_chat_message
    ...
    for i, msg in enumerate(st.session_state["history"]):
        if msg["role"] == "user":
            ...
            with st.chat_message("assistant"):
                ...
                # Activamos la voz SOLO para el último mensaje del asistente.
                if i == len(st.session_state["history"]) - 1 and st.session_state["voice_on"]:
                    # Usamos un pequeño delay para que la interfaz se cargue primero
                    time.sleep(0.5) 
